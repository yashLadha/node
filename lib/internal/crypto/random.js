...
function randomInt(min, max, callback) {
  ...
  if (!NumberIsSafeInteger(min)) {
    const errObj = new ERR_INVALID_ARG_TYPE('min', 'a safe integer', min);
    if (isSync) {
      throw errObj;
    } else {
  ...
  if (!NumberIsSafeInteger(max)) {
    const errObj = new ERR_INVALID_ARG_TYPE('max', 'a safe integer', max);
    if (isSync) {
      throw errObj;
    } else {
      return callback(errObj);
  ...
  if (max <= min) {
    const errObj = new ERR_OUT_OF_RANGE(
  ...
  if (!(range <= RAND_MAX)) {
    const errObj = new ERR_OUT_OF_RANGE(`max${minNotSpecified ? '' : ' - min'}`,
                                        `<= ${RAND_MAX}`, range);
    if (isSync) {
      throw errObj;
    } else {
